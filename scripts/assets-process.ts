...
function processHtmlFile(filePath: string): void {
  ...
  if (isCloudflarePage) {
    ...
    validateCloudflarePage($, filePath);
  } else {
...
function validateCloudflarePage($: cheerio.CheerioAPI, filePath: string): void {
  const route = getCloudflareRouteContext(filePath);
  const html = $.html();
  const externalAssets = [
    ...$('link[rel="stylesheet"]').toArray(),
    ...$("script[src]").toArray(),
    ...$('link[rel="preload"][as="script"]').toArray(),
    ...$('link[rel="modulepreload"]').toArray(),
  ];

  if (externalAssets.length > 0) {
    throw new Error(`Cloudflare page is not self-contained: ${filePath}`);
  }

  if (html.includes("/_next/static")) {
    throw new Error(
      `Cloudflare page still references _next/static: ${filePath}`,
    );
  }

  if (!html.includes("::RAY_ID::")) {
    throw new Error(`Cloudflare page is missing ::RAY_ID::: ${filePath}`);
  }

  if (route?.directory === "challenge") {
    const expectedToken =
      route.type === "javascript" || route.type === "non-interactive"
        ? "::IM_UNDER_ATTACK_BOX::"
        : "::CAPTCHA_BOX::";

    if (!html.includes(expectedToken)) {
      throw new Error(
        `Challenge page ${filePath} is missing required token ${expectedToken}`,
      );
    }
  }

  if (route?.directory === "error") {
    const expectedToken =
      route.type === "500s"
    ...
    if (!html.includes(expectedToken)) {
      throw new Error(
        `Error page ${filePath} is missing required token ${expectedToken}`,
      );
    }
  }
}
...
function inlineLocalStyles($: cheerio.CheerioAPI): void {
  ...
  $('style[data-inlined="yueyuan-cloudflare"]').remove();
...
function appendStandaloneLocaleScript(
  ...
  $('script[data-yueyuan-locale="true"]').remove();
...
function isRayFallback(value){return !value||value.indexOf("::RAY_ID::")!==-1||value==="7F4C-9A21-RAY"||value==="\\u7531 Cloudflare \\u751f\\u6210"||value==="Generated by Cloudflare"||value==="\\u5f85\\u751f\\u6210"||value==="Pending";}
function extractDetailRayId(){var box=document.querySelector(".yy-detail__box,.yy-widget__box");var match=((box&&box.textContent)||"").match(/Ray\\s*ID:\\s*([a-f0-9]{16,32})/i);return match?match[1]:"";}
function applyRayFallback(){var locale=document.documentElement.getAttribute("data-locale")==="zh"?"zh":"en";var fallback=locale==="zh"?"\\u5f85\\u751f\\u6210":"Pending";var detailRayId=extractDetailRayId();each("[data-ray-id]",function(node){var value=(node.textContent||"").replace(/^\\s+|\\s+$/g,"");if(isRayFallback(value)){node.textContent=detailRayId||fallback;}});}
function observeRaySource(){if(!("MutationObserver" in window)){return;}var observer=new MutationObserver(applyRayFallback);each(".yy-detail__box,.yy-widget__box",function(node){observer.observe(node,{characterData:true,childList:true,subtree:true});});}
...
function addCloudflareMetaTags($: cheerio.CheerioAPI): void {
  ...
  $(
    'meta[name="client-ip"], meta[name="ray-id"], meta[name="location-code"], meta[name="build-date"], meta[name="version"]',
  ).remove();
